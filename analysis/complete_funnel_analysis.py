#!/usr/bin/env python3
"""
ANALISI COMPLETA FUNNEL LDP - Dicembre 2025
Copre: Funnel, Creative, Setter, Venditori, Statistiche
"""
import json
import csv
...
def load_data():
    ...
    with open(DATA_DIR / "calls_full.json", 'r') as f:
        ...
        calls = [r for r in data['records']
                if DATE_START <= r.get('fields', {}).get('Data Call', '')[:10] <= DATE_END]
...
def print_report(funnel, ads, setters, closers):
    ...
    print("\n" + "="*80)
    print("📊 ANALISI COMPLETA FUNNEL LDP - DICEMBRE 2025")
    print("="*80)
    ...
    print(f"    ↓ ({funnel['cand_to_call_rate']:.1f}% conversion)")
    print(f"Calls:        {funnel['calls']:,}")
    ...
    print(f"\n📊 Breakdown per Stato:")
    for status, count in sorted(funnel['by_status'].items(), key=lambda x: -x[1]):
    ...
    # WORST CREATIVE
    print("\n\n❌ WORST 5 CREATIVE (CPL Alto)")
    print("-" * 80)
    worst_ads = sorted([a for a in ads if a['leads'] > 0], key=lambda x: -x['cpl'])[:5]
    ...
    print(f"{'Setter':<25} {'Calls':>6} {'Show Up':>8} {'Close':>6} {'SU%':>6} {'Close%':>7} {'Revenue':>10}")
    print("-" * 80)
    for setter, stats in top_setters:
        print(f"{setter[:24]:<25} {stats['calls']:>6} {stats['show_ups']:>8} "
              f"{stats['closures']:>6} {stats['show_up_rate']:>5.1f}% "
              f"{stats['close_rate']:>6.1f}% €{stats['revenue']:>9,.0f}")

    # Best/Worst setter by show up rate (min 10 calls)
    qualified_setters = {k: v for k, v in setters.items() if v['calls'] >= 10}
    ...
    print(f"{'Venditore':<25} {'Show Up':>8} {'Close':>6} {'Close%':>7} {'Revenue':>10} {'Ticket':>8}")
    print("-" * 80)
    for closer, stats in top_closers:
        print(f"{closer[:24]:<25} {stats['show_ups']:>8} "
              f"{stats['closures']:>6} {stats['close_rate']:>6.1f}% "
    ...
    # Creative insights
    best_ad = top_ads[0]
    worst_ad = worst_ads[0]
    print(f"🎨 CREATIVE:")
    print(f"   • Best: '{best_ad['name'][:50]}' (CPL €{best_ad['cpl']:.2f})")
    print(f"   • Worst: '{worst_ad['name'][:50]}' (CPL €{worst_ad['cpl']:.2f})")
    print(f"   • Differenza: {worst_ad['cpl'] / best_ad['cpl']:.1f}x")
    ...
    print(f"   • Call→Show Up: {funnel['call_to_showup_rate']:.1f}% (benchmark: 30-40%)")
    if funnel['call_to_showup_rate'] < 30:
        print(f"     ⚠️ BASSO - Migliora reminder e qualifica setter")
    print(f"   • Show Up→Close: {funnel['showup_to_close_rate']:.1f}% (benchmark: 25-35%)")
    if funnel['showup_to_close_rate'] < 25:
